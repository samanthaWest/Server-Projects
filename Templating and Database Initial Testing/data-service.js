...
searchbar : searchBar = (search) => {
    return new Promise(function(resolve,reject){
       // we will return this in resolve
       // with correct match if no match test for
       // an empty array send to 404 page >.<
        var temp = [];
        ...
        if(search.match(/[a-zA-Z]/)){
            ...
            // console.log(space); + 1 to pass space char
            var first = search.substring(0,space);
            var last = search.substring(space+1,length);        

        for(i in employees){
            // ensure upper case/lower case wont effect search condition
            var f1 = employees[i].firstName.toUpperCase();
            var f2 = first.toUpperCase();
            //console.log(f1);
            var l1 = employees[i].last_name.toUpperCase();
            var l2 = last.toUpperCase();
